<div>
    <div class="d-flex align-items-center mb-2">
        ...
        @if(  count($filtersSelected) || $search)
        ...
        @endforeach
{{--        <div class="ml-auto">--}}
{{--            <button type="button" class="btn btn-light ml-2" wire:click="download()">  {{trans('general.download')}}--}}
        ...
        @if($activitiesLog)
            <div class="table-responsive">
                ...
                <table class="table table-sm m-0">
                    ...
                    <tr>
                        <th class="w-10">{{ trans('general.date')}}</th>
                        <th class="w-10">{{ trans('general.user')}}</th>
                        <th class="w-10">{{ trans('general.email')}}</th>
                        <th class="w-20">{{ trans('general.action')}}</th>
                        <th class="w-20">{{ trans('general.table')}}</th>
                        <th class="w-5">{{ trans('general.id').'-'.trans('general.table')}}</th>
                        <th class="w-5">{{ trans('general.IP')}}</th>
                        <th class="w-15">{{ trans('general.date')}}</th>
                    ...
                    @foreach( $activitiesLog as $item)
                        <tr>
                            ...
                            <td>{{ $item->subject?$item->subject->getTable():'' }}</td>
                            <td>{{ $item->subject?$item->subject->id:'' }}</td>
                            @if( $item->properties && $item->getExtraProperty('browser_information'))
                                <td>
                                    {{$item->getExtraProperty("browser_information")['ip'] ??''}}
                                </td>
                            @else
                                <td></td>
                            @endif
                            ...
                            <td class="text-center">
                                <a href="javascript:void(0);" class="" aria-expanded="false"
                                   wire:click="$emitTo('poa.change-control.poa-change-control-detail', 'open', {{ $item->id }})"> <i
                                            class="fas fa-eye mr-1 text-info" data-toggle="tooltip" data-placement="top" title="" data-original-title="Ver detalles"></i>
                                </a>
                            </td>
                        </tr>
                    @endforeach

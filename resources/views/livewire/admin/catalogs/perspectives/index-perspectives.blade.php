            ...
            <div class="card">
                <div class="table-responsive">
                    <table class="table  m-0">
                        ...
                        <tbody>
                        @foreach($perspectives as $index=>$item)
...
@push('page_script')
    <script>
        Livewire.on('toggleCreatePerspective', () => $('#create-perspective').modal('toggle'));
        Livewire.on('toggleUpdatePerspective', () => $('#update-perspective').modal('toggle'));
        $('#update-perspective').on('show.bs.modal', function (e) {
            //get level ID & plan registered template detail ID
            let id = $(e.relatedTarget).data('item-id');
            //Livewire event trigger
            Livewire.emit('openPerspective', id);
        });
    </script>
    <script>

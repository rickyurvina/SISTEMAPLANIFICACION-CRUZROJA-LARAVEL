            ...
            <div class="d-flex ml-auto">
                @can('admin-crud-admin')
                    <button type="button" class="btn btn-success border-0 shadow-0" data-toggle="modal"
                            data-target="#create-unit">{{ trans('general.create')}}   {{ trans('general.module_units')  }}
                    </button>
                @endcan
            </div>
        </div>
                ...
                <div class="table-responsive">
                    <table class="table  m-0">
                        ...
                        <tr>
                            ...
                            <th class="w-15">
                                <a wire:click.prevent="sortBy('abbreviation')" role="button" href="#">
                                    {{ trans('general.abbreviation')}}
                                    <x-sort-icon sortDirection="{{$sortDirection}}" sortField="abbreviation"
                                                 field="{{$sortField}}"></x-sort-icon>
                                </a>
                            </th>
                        ...
                        @foreach($units as $index=>$item)
                            <tr wire:key="{{time().$index}}">
                                ...
                                <td class="text-center">{{ $item->is_for_people?'X':'' }}</td>
...
<div wire:ignore>
    <livewire:admin.catalogs.units.create-unit />
</div>
<div wire:ignore>
    <livewire:admin.catalogs.units.edit-unit />
</div>
@push('page_script')
    <script>
        Livewire.on('toggleCreateUnit', () => $('#create-unit').modal('toggle'));
        Livewire.on('toggleUpdateUnit', () => $('#update-unit').modal('toggle'));
        $('#update-unit').on('show.bs.modal', function (e) {
            //get level ID & plan registered template detail ID
            let id = $(e.relatedTarget).data('item-id');
            //Livewire event trigger
            Livewire.emit('openUnit', id);
        });
    </script>
    <script>

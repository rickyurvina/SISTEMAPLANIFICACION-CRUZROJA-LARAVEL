<div>
    <div wire:ignore.self class="modal fade" id="poa-edit-rescheduling" tabindex="-1" role="dialog" aria-hidden="true" data-backdrop="static" data-keyboard="false">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <x-form.modal.select id="phase" label="{{ trans('general.phase') }}" class="col-6" wire:model="phase">
                            ...
                            @foreach(\App\Models\Poa\Poa::PHASES  as $item)
                        ...
                        <x-form.modal.select id="state" label="{{ trans('general.state') }}" class="col-6">
                            ...
                            @foreach($arrayStates  as $area)
                                <option value="{{$area}}" {{$this->state==$area ? 'selected':''}}> {{$area}}</option>
                            @endforeach
                        </x-form.modal.select>
                    </div>
                </div>

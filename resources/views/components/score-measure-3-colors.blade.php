@props(['id', 'score' => 0, 'title' => '', 'red' => '', 'goal' => '', 'performance' => '', 'difPerformance' => '', 'difPerformanceColor' => '', 'beforePeriod' => '', 'hasData' => false])
    ...
    <div class="card-body">
        @if($score)
            <div class="d-flex justify-content-between">
                <span class="fs-4x ml-6 font-weight-bold" style="z-index: 9999"
                      data-toggle="popover" data-trigger="hover" data-placement="top"
                      data-content="Valor actual">
                        {{ $performance }}
                    </span>
                @if($hasData)
                    <span class="badge fs-4x text-white mr-6" style="z-index: 9999; background-color: {{ $difPerformanceColor }}"
                          data-toggle="popover" data-trigger="hover" data-placement="top"
                          data-content="Diferencia con {{ $beforePeriod }}"
                          style="background-color: {{ $difPerformanceColor }}">
                        {{ $difPerformance }}
                    </span>
                @endif
            </div>
        @else
            <div class="d-flex justify-content-center">
                <span class="badge badge-dark fs-2x">Aún no actualizado</span>
            </div>
        @endif
        <div id="score-chart" style="height: 265px;width: 350px; position: relative; top: -30px"></div>
    </div>
</div>
...
@push('page_script')
    <script>
        am4core.ready(function () {
            ...
            // Add bottom label
            let leftLabel = chart.chartContainer.createChild(am4core.Label);
            leftLabel.isMeasured = false;
            leftLabel.textAlign = "middle";
            leftLabel.text = "[font-size:25px; #000000; bold]{{ $red }}\n[font-size:15px; #8b98a6; bold]ROJO";
            leftLabel.y = am4core.percent(82);
            leftLabel.x = am4core.percent(5);
            ...
            let rightLabel = chart.chartContainer.createChild(am4core.Label);
            rightLabel.isMeasured = false;
            rightLabel.textAlign = "middle";
            rightLabel.text = "[font-size:25px; #000000; bold;]{{ $goal }}\n[font-size:15px; #8b98a6; bold]META";
            rightLabel.y = am4core.percent(82);
            rightLabel.x = am4core.percent(68);
            @else
        ...
        window.addEventListener('updateData', event => {
            am4core.ready(function () {
                ...
                if(event.detail.score) {
                    ...
                    leftLabel.isMeasured = false;
                    leftLabel.textAlign = "middle";
                    leftLabel.text = "[font-size:25px; #000000; bold]" + event.detail.red + "\n[font-size:15px; #8b98a6; bold]ROJO";
                    leftLabel.y = am4core.percent(82);
                    leftLabel.x = am4core.percent(5);
                    ...
                    rightLabel.isMeasured = false;
                    rightLabel.textAlign = "middle";
                    rightLabel.text = "[font-size:25px; #000000; bold;]" + event.detail.goal + "\n[font-size:15px; #8b98a6; bold]META";
                    rightLabel.y = am4core.percent(82);
                    rightLabel.x = am4core.percent(68);
                }else {
                    let range10 = axis.axisRanges.create();

...
class CreateDeleteTasksOfProject
{
    ...
    public function handle(ProjectSubsidiaryUpdated $event)
    {
        ...
        $primaryTasks = Task::where('project_id', $project->id)
            ->where('type', 'task')
            ->whereNotIn('company_id',$subsidiaries->pluck('company_id')->toArray())
            ->get();
        foreach ($subsidiaries->pluck('company_id') as $item) {
            $existTask = Task::withoutGlobalScope(\App\Scopes\Company::class)
